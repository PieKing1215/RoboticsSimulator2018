...
import com.jmr.wrapper.client.ClientConfig;
...
/**
 * A slightly modified version of the original that allows the timeout to be changed.
 */
...
public class MyClient extends Client implements IProtocol {
	 ...
	 */
	public MyClient(String address, int tcpPort, int udpPort) {
		try {
	...
	public void connect() {
		try {
			...
			tcpSocket.connect(new InetSocketAddress(address, tcpPort), 100);
